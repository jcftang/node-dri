...
function createDirectory(path, uploadsDirectory, onSuccess, onError) {
	fs.mkdir(uploadsDirectory + path, 0777, true, function(err) {
		if(err) {
			console.log(err);
			onError(err)
		} else {
...
exports.uploadFile = function uploadFile(files, uploadsDirectory, onSuccess, onError) {
	if(files.upload.size != 0) {
		createDirectory('', uploadsDirectory, function() {
