...
exports.approveItem = function approveItem(id, namespace, onSuccess, onError) {
	...
	this.getObject(id, function(data) {
		fedora.createFedoraObject(namespace, data.properties.titleInfo[0].title, function(response) {
			var pid = response;
...
exports.uploadFile = function uploadFile(files, onSuccess, onError) {
	...
	if(files.size != 0) {
		createDirectory('', dri.config.uploadDirectory, function() {
			...
			var os = fs.createWriteStream(dri.config.uploadDirectory + files.name);
			...
			util.pump(is, os, function() {
				fs.unlink(files.path, function(err) {
					...
					onSuccess(dri.config.uploadDirectory + files.name);
				});
			});
		}, function(err) {
			onError(err)
