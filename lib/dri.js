/**
 * @author mvanwamb
 * @author Quirijn Groot Bluemink
 */

var Mongolian = require("mongolian");
var ObjectId =  require('mongolian').ObjectId   // new ObjectId(byteBuffer or hexString)
var helper = require('./helpers');
var fs = require('fs');
var image;


exports.updateItem = function(req){

	var meta = req.body;
	var files = req.files;

	//console.log(items);
	helper.update(meta,files);
	//gridfs();
}


/*
   Function: loadImg

   Searches for an images and displays it.

   Parameters:

      id - the id of the file
      name - the filenameshutdownServer
      res - the view object (res)

*/
exports.loadImg = function loadImg(id,name,res){
	//setup connection with mongodb
	var server = new Mongolian
	db = server.db("mydb");
	items = db.collection("items")
	//get gridfs object
	var gridfs = db.gridfs()// name defaults to 'fs'
	//search for a file with a certain name and an id
	//new ObjectId coverts the String version of the id into a byte one
	gridfs.findOne({filename:name,_id:new ObjectId(id)}, function(err, file) {
			//sends back the image to the view
			if (!err && file) {
				res.writeHead(200, {'Content-Type': 'image/jpeg'});
                var stream = file.readStream();
                //incase the file couldn't be loaded it logs an error
                stream.on("error", function(err){
                    console.log(err); 
                    
                }).pipe(res);
            }
            //runs if there is an error with finding the file
            else
            {
                res.writeHead(404, {'Content-Type': 'text/plain'});
                res.write('404 Not Found\n');
                res.end();
            }
		})
}


/*
   Function: findImages

   Searches for all files linked to a metadata object

   Parameters:

      req - request objectshutdownServer
      res - result object

   Returns:

      array of the corresponding files.
*/
exports.findImages = function findImages(fileId,callback){
	//setting up connection with the server
	items = helper.getDatabaseCollection("series");
	var gridfs = db.gridfs()
	var files = new Array();
	//search for a file with the id that is provided in the request (in url)
	gridfs.find({metadata:{id:fileId}}).forEach(function(file) {
		//converting the id from bytes to a string id
		file._id = file._id.toString();
		//create a small object which contains the filename and the id of the file.
		var data = {};
		data._id = file._id;
		data.filename = file.filename;
		data.contentType = file.contentType;
		//add it to the array of files
		files.push(data);
		//executes this function when forEach is ready
	}, function() {
		//send back the files to the client
		callback(files);
	});

}


//gets all the files related to an item
exports.getAll = function getAllItems(callback){
	//setup a connection
	items = helper.getDatabaseCollection("series");
	
	var gridfs = db.gridfs()// 
	var arr = new Array();
	gridfs.find().forEach(function(file) {
		// do something with a single post
		arr.push(file);
	}, function() {
		callback(arr);
	})


}


exports.edit = function edit(callback){

		helper.getItemsByType("serie",function(data){
			var array = new Array();
			array = data;
			callback(data);
		});	

}
/*
   Function: getAllSeries

   Sends back all series to the request

   Parameters:

      req - request object
      res - result object
*/
exports.getAllSeries = function getAllSeries(callback) {
		helper.getItemsByType("serie",function(data){
			callback(data);
		});
}

exports.getAllCollections = function getAllCollections(callback) {
		helper.getItemsByType("collection",function(data){
			callback(data);
		});
}

exports.getAllItems = function getAllItems(callback) {
		helper.getItemsByType("item",function(data){
			callback(data);
		});
}


/*
   Function: getItems

   Searches for all files linked to a metadata object

   Parameters:

      req - request object
      res - result object

   Returns:

      array of the corresponding files.
*/
exports.getItems = function getItems(id,callback){

	items = helper.getDatabaseCollection("series");
			
	items.find({parentId:id}).sort({ created: 1 }).toArray(function (err, array) {
	for(item in array){
			array[item]._id = array[item]._id.toString();
		}
		callback(array);
	})
}


exports.getItem = function(id,callback){

	items = helper.getDatabaseCollection("series");
			
	items.findOne({_id:new ObjectId(id)},function (err, array) {
			array._id = array._id.toString();
		callback(array)
	})
}


exports.createSeries = function(req,callback){

	var items = helper.getDatabaseCollection("series");
	var rootItem = {}
	var files = req.body;
	
	rootItem.type = "serie"
	rootItem.parentId = files.collection;
	for(var i in files) {
		if(i != "collection"){
			rootItem[i] = files[i];
		}
	}
	items.insert(rootItem, function(err, value) {
		if(err) {
			console.log(err);
			
			return -1
		} else {
			callback();
			return 0
		}

	});
	


}

exports.createCollection = function(req,callback){

	var items = helper.getDatabaseCollection("series");
	var rootItem = {}
	var files = req.body;
	
	rootItem.type = "collection"
	for(var i in files) {
			rootItem[i] = files[i];
	}
	items.insert(rootItem, function(err, value) {
		if(err) {
			console.log(err);
			
			return -1;
		} else {
			callback();
			return 0;
		}

	});
	

}
exports.createitem = function(req,callback){
	var item = {};
	var items = helper.getDatabaseCollection("series");
	var files = req.body;
	for(var i in files) {
		if(i != 'amount' && i!= "series") {
			item[i] = files[i];
		}

	}
	if(files.collection == "series"){
		console.log("-series-")
		item.parentId = files.serieSelect;
	}
	if(files.collection == "collections"){
		console.log("-collections-")
		item.parentId = files.collectionSelect;
	}
	if(files.collection == "items"){
		console.log("-items-")
		item.parentId = files.itemSelect;
	}

	item.type = "item";
	for(var i = 0; i < req.body.amount; i++) {
		item._id = new ObjectId();
		item.objectId = i + 1;
		items.insert(item, function(err, value) {
			if(err) {
				console.log(err);
			}else{
				callback();
			}
		});
	}
}


exports.removeItem = function(id,callback){
	items = helper.getDatabaseCollection("series");
	items.remove({_id:new ObjectId(id)},function(err,value){
		if(err){
			console.log(err);
		}
	
	})
		items.remove({parentId:req.params.id},function(err,value){
		if(err){
			console.log(err);
		}

	})

	callback();
}
