...
exports.lastCreatedByType = function lastCreatedByType(objType,onSuccess, onError) {
	schemas.driObject.find({type:objType}).sort('dateCreated',-1).limit(5).execFind(function(err, doc) {
		if(err) {
			onError(err)
		} else {
			onSuccess(doc)
...
exports.lastEdited = function lastEdited(onSuccess, onError) {
	schemas.driObject.find({dateModified : { $exists : true }}).sort('dateModified',-1).limit(5).execFind(function(err, doc) {
		if(err) {
			onError(err)
		} else {
			onSuccess(doc)
...
exports.lastEditedByType = function lastEditedByType(objType,onSuccess, onError) {
	schemas.driObject.find({type:objType,dateModified : { $exists : true }}).sort('dateModified',-1).limit(5).execFind(function(err, doc) {
		if(err) {
			onError(err)
		} else {
			onSuccess(doc)
