...
exports.lastCreated = function lastCreated(onSuccess, onError) {
	schemas.driObject.find().sort('dateCreated',-1).limit(10).execFind(function(err, doc) {
		if(err) {
			onError(err)
		} else {
			onSuccess(doc)
...
exports.lastCreatedByType = function lastCreatedByType(objType,onSuccess, onError) {
	schemas.driObject.find({type:objType}).sort('dateCreated',-1).limit(10).execFind(function(err, doc) {
		if(err) {
			onError(err)
		} else {
			onSuccess(doc)
...
exports.lastEdited = function lastEdited(onSuccess, onError) {
	schemas.driObject.find().sort('dateModified',-1).limit(10).execFind(function(err, doc) {
		if(err) {
			onError(err)
		} else {
			onSuccess(doc)
...
exports.lastEditedByType = function lastEditedByType(objType,onSuccess, onError) {
	schemas.driObject.find({type:objType}).sort('dateModified',-1).limit(10).execFind(function(err, doc) {
		if(err) {
			onError(err)
		} else {
			onSuccess(doc)
