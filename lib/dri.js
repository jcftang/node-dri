...
exports.removeItem = function(id, onSuccess, onError) {
	...
	items.remove({_id : new ObjectId(id)}, function(err, value) {
		if(err) {
			...
			var parent = {};
			parent._id = id;
			removeChild([parent],id,onSuccess,onError);
		}
	});
}
...
function removeChild(parentArray, id, onSuccess, onError) {
	if(parentArray.length > 0) {
		var items = helper.getDatabaseCollection("series")
		for(var i = 0; i < parentArray.length; i++) {
			items.find({
				parentId : parentArray[i]._id.toString()
			}).toArray(function(err, array) {
					if(array.length == 0){
						onSuccess(id);
					}
					for(var j in array) {
						items.remove({
							...
							if(err) {
								...
								removeChild(array, id, onSuccess, onError);
							}
						});
					}
			});
