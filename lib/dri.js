...
var dri = this
var uploadsDirectory = '/home/qgrootbl/Develop/node-dri/binary/'
...
exports.saveBinary = function saveBinary(data, onSuccess, onError) {
	createDirectory('/proj2', function(result) {
		createFile('/proj2', function(result) {
			onSuccess(result)
		}, function(err) {
			onError(err)
		})
...
function createDirectory(path, onSuccess, onError) {
	fs.mkdir(binaryPath + path, 0777, true, function(err) {
		if(err) {
			console.log(err);
			onError(err)
		} else {
...
function createFile(path, onSuccess, onError) {
	fs.writeFile(binaryPath + path + "/test.txt", "Hey there!", function(err) {
		if(err) {
			console.log(err);
			onError(err)
		} else {
...
exports.uploadFile = function uploadFile(req, onSuccess, onError) {
	...
	form.parse(req, function(err, fields, files) {
		...
		var os = fs.createWriteStream(uploadsDirectory + files.upload.name);
		...
		util.pump(is, os, function() {
			fs.unlink(files.upload.path, function(err) {
				...
				onSuccess(uploadsDirectory + files.upload.name);
			});
		});
	});
}
