...
exports.toDC = function(json) {
	...
	if(properties) {
		...
		_.each(properties.originInfo, function(val, key, list) {
			...
			_.each(val, function(val, key, list) {
				if(key == "publisher") {
					...
					if(key.indexOf("dateOther") != -1) {
						_.each(val, function(val, key, list) {
							infoTag += "<dc:date>" + val.point + " " + val.dateOther + "</dc:date>"
						})
					}
				}
			})
		...
		_.each(properties.physicalDescription, function(val, key, list) {
			...
			_.each(val, function(val, key, list) {
				if(key.indexOf("internetMediaType") != -1) {
					_.each(val, function(val, key, list) {
						infoTag += val.internetMediaType + ", "
					})
				}else if(key.indexOf("type") != -1) {
					_.each(val, function(val, key, list) {
						infoTag += val.type + ":" + val.typeDescription + ", "
					})
				} else {
					infoTag += val + ", "
				}
		...
		_.each(properties.subject, function(val, key, list) {
			...
			_.each(val, function(val, key, list) {
				if( typeof val == "object") {
					_.each(val, function(val, key, list) {
						infoTag += val.topic + " " + val.authority + ", 	"
					})
				} else {
					infoTag += val + ", "
				}
